...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NAUTOBOT_FIREWALL_MODELS_xxx
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.nautobot_firewall_models.compose_http_timeout,
    ...
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.nautobot_firewall_models.project_name}",
        f'--project-directory "{context.nautobot_firewall_models.compose_dir}"',
    ]
    ...
    if (
        context.nautobot_firewall_models.ephemeral_ports
        and context.nautobot_firewall_models.compose_files == ORIGINAL_COMPOSE_FILES
    ):
        compose_file_path = os.path.join(
            context.nautobot_firewall_models.compose_dir, "docker-compose.ephemeral-ports.yml"
        )
        compose_command_tokens.append(f' -f "{compose_file_path}"')
...
def _ensure_creds_env_file(context):
    ...
    if not os.path.exists(os.path.join(context.nautobot_firewall_models.compose_dir, "creds.env")):
        ...
        shutil.copy(
            os.path.join(context.nautobot_firewall_models.compose_dir, "creds.example.env"),
            os.path.join(context.nautobot_firewall_models.compose_dir, "creds.env"),
        )
...
def pylint(context, target=None, recursive=False):
    ...
    if recursive:
        command += " --recursive=y"
    command += f" {' '.join(target) if target else 'nautobot_firewall_models'}"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    app_dir = Path(__file__).absolute().parent / Path("nautobot_firewall_models")
    migrations_dir = app_dir / Path("migrations")
    migrations_target_module = "nautobot_firewall_models.migrations"
...
def djhtml(context, check=False):
    ...
    command = "djhtml -t 4 nautobot_firewall_models/templates/"
...
def generate_test_data(context, flush=False, database=None):
    """Generate test data in Nautobot for Nautobot Firewall Models."""
    command = "nautobot-server generate_nautobot_firewall_models_test_data"
    if database:
